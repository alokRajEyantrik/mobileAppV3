<script lang="ts">
  ...
  interface Question {
    id: string;
    type: 'text' | 'radio' | 'select' | 'checkbox' | 'textarea' | 'date' | 'number' | 'multiple-select';
    question: string;
    bindsTo?: string;
    bindsTo_template?: string;
    options?: Array<{ label: string | { var: string }; value: string | { var: string } | number | boolean }>;
    required?: boolean;
    showWhen?: any; // JSON Logic expression
    validation?: { condition: any; message: string };
    errorMessage?: Record<string, string>;
    uiMeta?: { readonly?: boolean; placeholder?: string; rows?: number; min?: string | number; max?: string | number; step?: number | 'any' };
  }
  ...
  function getOptionValue(value: string | { var: string } | number | boolean): string | number | boolean {
  ...
  function updateAnswerByKey<T extends string | number | boolean | (string | number)[]>(key: string, value: T): void {
  ...
  function validateGSTState(gstNumber: string): string | null {
    ...
    if (!pincode_IN_Selected[stateName as keyof typeof pincode_IN_Selected]) return 'stateNotServed';
  ...
  function updateAnswer(question: Question, value: string | number | boolean | (string | number)[]): void {
  ...
  function getValidationErrorMessage(question: Question, answers: Answers): string | null {
    ...
    if ((val === undefined || val === null || (typeof val === 'string' && val === '') || (Array.isArray(val) && val.length === 0)) && currentPageIndex === 0) {
      return null;
    }
    ...
    if (question.validation?.condition) {
      ...
      if (isInvalid) {
        ...
        if (validatorFnName && typeof validators[validatorFnName as keyof typeof validators] === 'function') {
          const errorKey = (validators[validatorFnName as keyof typeof validators] as any)(
            question.type === 'multiple-select' ? (val as (string | number)[]) : val?.toString() ?? ''
          );
          if (errorKey) {
...
<div class="max-w-4xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div class="mb-6">
        ...
        {:else if question.type === 'select' }
          <SelectField
            id={question.id}
            label={question.question}
            options={question.options?.map(opt => ({ label: opt.label as string, value: opt.value as string | number })) ?? []}
        ...
        {:else if question.type === 'number'}
          <NumberField
            id={question.id}
            label={question.question}
            value={currentAnswers[resolveBindsTo(question, combinedAnswers, selectedLoan)] as number | null ?? null}
  ...
  <div class="bg-gray-900 text-white p-6 rounded-lg shadow-md">
    ...
    <pre class="bg-gray-800 p-4 rounded-md overflow-auto">{JSON.stringify(currentAnswers, null, 2)}</pre>
    <h3 class="text-lg font-semibold mb-4 mt-6">Debug: combinedAnswers</h3>
    <pre class="bg-gray-800 p-4 rounded-md overflow-auto">{JSON.stringify(combinedAnswers, null, 2)}</pre>
