<script lang="ts">
	...
	import formSchema from '$lib/config/plot-loan-schema.json';
	...
	$: propertyCityOptions = getCityOptionsForState(currentAnswers['propertyStateName']);
	$: residenceCityOptions = getCityOptionsForState(currentAnswers['residenceStateName']);

	// Business address fields visibility
	$: showBusinessAddress = currentAnswers['residenceOptionSame'] === 'No';
	...
	function updateAnswer(
		...
		} else if (key === 'residenceOptionSame' && value === 'Yes') {
	...
	function getValidationErrorMessage(question: Question, answers: Answers): string | null {
		...
		if (question.id.startsWith('q4_residence') || question.id.startsWith('q5_residence')) {
			const addressSameOrNot = answers['residenceOptionSame'];
				...
				{:else if question.type === 'derivedSelect'}
					<DerivedSelect
						...
						disabled={(question.id === 'q2_propertyCityName' &&
							...
							(question.id === 'q5_residenceCityName' && !currentAnswers['residenceStateName'])}
					/>
